    ...
    update(dt) {
        ...
        if (this.repeatH) {
            ...
            if (this.speed.x > 0 && (this.position.x > this.stageWidth)) {
                this.position.x = this.temp.position.x - this.texture.sizeWidth
            }
            // 向左移出屏幕，重新拼接到克隆体后面
            if (this.speed.x < 0 && (this.position.x < -this.texture.sizeWidth)) {
                this.position.x = this.temp.position.x + this.temp.texture.sizeWidth
            }
            // 克隆体向右移出屏幕，重新拼接到本体前面
            if (this.temp.speed.x > 0 && (this.temp.position.x > this.stageWidth)) {
                this.temp.position.x = this.position.x - this.temp.texture.sizeWidth
            }
            // 克隆体向左移出屏幕，重新拼接到本体前面
            if (this.temp.speed.x < 0 && (this.temp.position.x < -this.temp.texture.sizeWidth)) {
                this.temp.position.x = this.position.x + this.texture.sizeWidth
            }
        }
        // 纵向重复
        if (this.repeatV) {
            ...
            if (this.speed.y > 0 && (this.position.y > this.stageHeight)) {
                this.position.y = this.temp.position.y - this.texture.sizeHeight
            }
            // 向上移出屏幕，重新拼接到克隆体下面
            if (this.speed.y < 0 && (this.position.y < -this.texture.sizeHeight)) {
                this.position.y = this.temp.position.y + this.temp.texture.sizeHeight
            }
            // 克隆体向下移出屏幕，重新拼接到本体上面
            if (this.temp.speed.y > 0 && (this.temp.position.y > this.stageHeight)) {
                this.temp.position.y = this.position.y - this.temp.texture.sizeHeight
            }
            // 克隆体向上移出屏幕，重新拼接到本体下面
            if (this.temp.speed.y < 0 && (this.temp.position.y < -this.temp.texture.sizeHeight)) {
                this.temp.position.y = this.position.y + this.texture.sizeHeight
            }
        }
    }
    draw(ctx) {
